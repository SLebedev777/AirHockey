...
{
    MainMenuSettingsLayer* pRet = new (std::nothrow) MainMenuSettingsLayer();
    if (pRet && pRet->init())
    {
        pRet->autorelease();
    }
...
bool MainMenuSettingsLayer::init()
{
    ...
    auto text_title = ui::Text::create("SETTINGS", font_filename, font_size);
...
{
    EventCustom custom_event("event_main_menu_settings_layer_close");
